...
- `speed` is per-instance overrideable; unwritten reads return the shared default.
- `name`, `clip`, `wrapMode`, `clipStartTime`, `clipEndTime` read through to the shared asset.
