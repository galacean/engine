...
- `state.def` — the underlying shared `AnimatorStateDef` asset. Mutating fields like `def.wrapMode` mutates the shared `AnimatorController` asset and therefore affects every `Animator` using this controller.
...
// Shared asset configuration (broadcast to every Animator using this controller)
state.def.wrapMode = WrapMode.Once;
```
