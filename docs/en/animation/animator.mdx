...
// Per-instance playback speed override (only affects this Animator instance).
playData.speed = 0;
// Restore live binding to the shared state.speed.
playData.clearSpeedOverride();
```
...
- `playData.speed` — per-Animator playback speed override. Reading and writing it only affects this `Animator` instance and won't affect other `Animator` instances sharing the same `AnimatorController`. Call `clearSpeedOverride()` to resume live binding to `state.speed`.
