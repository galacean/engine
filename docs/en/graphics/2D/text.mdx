...
| `Font` | Custom font |
| `Width` | The width of the text in 3D space, used to turn on the line break mode. The displayed text will be wrapped and horizontally aligned according to this width. |
| `Height` | The height of the text in 3D space, when the total height of the text exceeds the set Height, it is processed with the OverflowMode setting and vertical alignment is used |
