...
After creating the ShaderLab source above (e.g. in the editor's `.shader` file), register it in code:

```ts filename="CustomPostProcessPass.ts"
Shader.create(grayScaleShaderSource); // Pass the ShaderLab string shown above
