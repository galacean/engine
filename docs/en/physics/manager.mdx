...
- Entity must add [collider](/en/docs/physics/collider/overview) components to be detected by raycasting
- When a ray hits multiple [collider shapes](/en/docs/physics/collider/colliderShape) at the same distance, it will return the Entity of the collider shape that was added first
- It is recommended to use [InputManager](/en/docs/input/input/) to handle input, as it provides a more convenient way to handle input
