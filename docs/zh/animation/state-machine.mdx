...
#### 动画状态
代表动画状态机中的一个单独状态，即某一时刻动画系统中播放的动画。，比如“站立”、“跑步”或“跳跃”。每个状态都有一个对应的动画片段。完整的API请参考[AnimatorState](/apis/core/#AnimatorState)。
...
#### 动画过渡
用于定义动画状态机中两个状态之间的转换规则和条件。它决定了何时以及如何从一个 AnimatorState 切换到另一个 AnimatorState。完整的API请参考[AnimatorStateTransition](/apis/core/#AnimatorStateTransition)。
...
##### 为动画过渡添加条件
你可以通过 [animatorStateTransition.addCondition](/apis/core/#AnimatorStateTransition-addCondition) 方法为动画过渡添加条件
