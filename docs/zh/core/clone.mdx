...
“默认深克隆”是一项显式能力，不会根据方法名推断。自定义数据类可通过继承 [DataObject](/apis/core/#DataObject) 加入：引擎会逐字段克隆其自身可枚举的字符串键属性。引擎数学值类型则由内部显式注册，并通过各自的 `copyFrom` 实现复制；自定义类仅仅声明一个名为 `copyFrom` 的方法并不会自动加入。此类对象在没有兼容预置实例时必须支持无参构造：引擎会先通过 `new Type()` 创建实例再填充，裸构造失败会直接报错。
...
| [deepClone](/apis/core/#deepClone) | 深克隆字段子树中支持深克隆的值。深意图会传播进可按字段克隆的成员；资产保持共享、实体引用重映射、运行时状态保留克隆体自己的，未注册的不透明成员保持默认赋值行为。如果被装饰的值本身无法满足深克隆，例如 `Entity` / `Component`、资产、函数、运行时状态对象或不透明的平台对象，克隆会直接报错。 |
...
console.log(script.c[0]); // output is (2,2,2)，因为 assignmentClone 让 c 与 cloneScript.c 使用同一个数组引用。
console.log(script.d[0]); // output is (1,1,1)，因为 deepClone 让 d[0] 与 cloneScript.d[0] 使用不同引用。
```

- 注意:

  - 字段装饰器优先级最高，并且在克隆走字段遍历的任何地方对嵌套对象的字段同样生效。
  - `deepClone` 会传播进容器和可按字段克隆的成员。没有深克隆默认的普通类实例会按字段复制，因此必须支持无参构造。资产保持共享，实体引用被重映射，运行时状态保留克隆体自己的，未注册的不透明成员保持共享，嵌套字段上的装饰器依然优先。
  - 字段克隆采用刻意收窄的契约：只有对象自身可枚举的字符串键属性具有结构克隆语义。私有字段、不可枚举属性、Symbol 键以及原型或访问器状态不会被重建；引擎也不承诺识别运行时中所有可能的不透明状态。
  - `deepClone` 无法直接克隆引擎绑定或不透明的值：对 `Entity` / `Component` 引用、资产、引擎运行时状态、函数或未注册的平台对象使用 `@deepClone` 会直接报错，因为这个显式意图无法被满足。请移除装饰器以回退到默认行为，或使用该值自身的克隆 API 得到真正的拷贝。
