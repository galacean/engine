...
| `FontSize` | 文本的字体大小 |
| `Font` | 自定义字体 |
| `Width` | 文本在三维空间中的宽: 1、决定了文本在开启换行模式时的显示宽度。当文本宽度达到或超过该设定值时，文本会自动换行至下一行继续显示 2、在水平对齐时，也是依据这个宽度来确定文本的水平位置关系。例如，左对齐时，文本段落从左侧边界开始，以该宽度为最大行宽显示多行文本；居中对齐时，每行文本会以这个宽度为参考，在容器内居中显示，确保整体文本在水平方向均衡分布 |
| `Height` | 文本在三维空间中的高: 1、当文本的总高度超出这一设定值时，会根据所选的溢出模式（Overflow Mode）进行相应的处理，例如截断显示、溢出显示 2、在竖直对齐时，也是依据这个高度来确定文本的竖直位置关系。例如，顶对齐时，文本段落从上侧边界开始；居中对齐时，文本会以这个高度为参考，在容器内居中显示，确保整体文本在竖直方向均衡分布|
| `LineSpacing` | 行间距 |
