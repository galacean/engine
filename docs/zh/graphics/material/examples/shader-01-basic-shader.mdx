...
本教程将通过一个简单的纹理渲染着色器，介绍 Galacean Shader 的基础语法结构，通过这个基础示例，你将学会：
- Galacean Shader 的基本结构
- 如何声明变量和结构体
- 顶点着色器的基本变换
- 片元着色器的纹理采样
- UV 坐标的传递和使用
...
Galacean Shader 采用三层嵌套结构：`Shader` → `SubShader` → `Pass`
...
前往 [游乐场](/examples/shader-01-basic-shader)