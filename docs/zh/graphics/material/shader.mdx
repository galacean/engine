...
`Galacean Shader` 是 Galacean 针对 Shader 自主研发的一套描述语言，它提供了一套完整的语法来定义着色器的结构、属性、渲染状态和着色器代码。通过 Galacean Shader，开发者可以创建从简单到复杂的各种视觉效果。

## 什么是 Galacean Shader

`Galacean Shader` 是一种声明式的着色器描述语言，它将着色器的各个组成部分组织成模块化的结构。与传统的 GLSL/HLSL 不同，Galacean Shader 不仅包含着色器代码，还包含材质属性定义、UI界面配置、渲染状态设置等完整信息。

### 解决的问题

`Galacean Shader` 框架主要解决以下传统着色器开发中的痛点：
...
Shader 模块是 Galacean Shader 的根模块，定义着色器的基本信息和全局设置：
...
```glsl showLineNumbers
#include "Common/Common.glsl"      // 通用函数
#include "Common/Light.glsl"       // 光照计算
#include "Shadow/Shadow.glsl"      // 阴影计算
#include "Common/Fog.glsl"         // 雾效计算
...
`UIScript` 是 `Galacean Shader` 的核心特性之一，允许通过 TypeScript 脚本实现智能的属性面板交互，包括设置宏开关、渲染状态等：
