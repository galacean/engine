...
编写好上面的 Galacean Shader 源码后（如在编辑器中创建 `.shader` 文件），在代码中注册：

```ts filename="CustomPostProcessPass.ts"
Shader.create(grayScaleShaderSource); // 传入上面的 Galacean Shader 字符串
...
可以看到，我们在自定义管线的 `onRender` 钩子中，不断设置融合后的强度，然后在 shader 中消费这个数据即可。只需在 Pass 中添加 `u_intensity` uniform 并修改片元着色器：
