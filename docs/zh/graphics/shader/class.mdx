...
此外还可以通过 [Camera.setReplacementShader](/engine/apis/galacean/#Camera-resetReplacementShader)，指定渲染管线的`replacementTag`，实现对设置指定 `Tag` 的 `SubShader` 替换的目的。

Tags可以通过 [SubShader.setTag](/engine/apis/galacean/#SubShader-setTag) 和 [ShaderPass.setTag](/engine/apis/galacean/#ShaderPass-setTag)进行指定，ShaderLab指定Tag方式详见[文档](./lab)
