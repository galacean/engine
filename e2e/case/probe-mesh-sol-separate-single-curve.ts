/**
 * @title SOL probe probe-mesh-sol-separate-single-curve
 * @category Particle
 */
import {
  Burst,
...
WebGLEngine.create({ canvas: "canvas" }).then((engine) => {
  ...
  sizeOverLifetime.sizeX = new ParticleCompositeCurve(new ParticleCurve(new CurveKey(0, 1.2), new CurveKey(1, 2.0)));
  sizeOverLifetime.sizeY = new ParticleCompositeCurve(new ParticleCurve(new CurveKey(0, 1.0), new CurveKey(1, 3.0)));
  sizeOverLifetime.sizeZ = new ParticleCompositeCurve(new ParticleCurve(new CurveKey(0, 0.9), new CurveKey(1, 1.5)));
