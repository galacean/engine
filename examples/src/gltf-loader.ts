...
WebGLEngine.create({ canvas: "canvas" }).then((engine) => {
  ...
  class MoveScript extends Script {
    ...
    onUpdate(deltaTime: number): void {
      ...
      transform.rotate(this._tempVec.set(0, 1, 0));
    ...
    onLateUpdate(deltaTime: number): void {
      ...
      if (length > 0) {
        ...
        for (let i = 1; i < length; i++) {
          ...
          BoundingBox.merge(boundingBox, renderer.bounds, boundingBox);
        }
  ...
  engine.resourceManager
    .load<GLTFResource>(
      "https://mdn.alipayobjects.com/chain_myent/uri/file/as/mynftmerchant/202512290332590116.gltf"
    )
    .then((glTF) => {
