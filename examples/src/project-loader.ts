...
async function start() {
  ...
  const p = engine.resourceManager.load({
    url: url.toString(),
  ...
  p.onProgress(
    ...
    (url: string, loaded: number, total: number) => {
      ...
      // updateDetailedProgress(url, percentage);
    }
  );

  try {
    await p;
    console.log("All project files loaded successfully!");
    console.log("Scene has been set as active scene");
  } catch (error) {
