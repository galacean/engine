...
export class ASpriteRenderer extends NodeAbility {
  ...
  protected setRect(rect?) {
    this._rect = rect || {
      x: 0,
      y: 0,
      width: this._texture ? (this._texture.image ? this._texture.image.width : this._texture.width) : 0,
      height: this._texture ? (this._texture.image ? this._texture.image.height : this._texture.height) : 0
    };
  }
