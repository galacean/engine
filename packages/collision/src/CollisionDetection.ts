...
export class CollisionDetection extends Script {
  ...
  _getWorldBox(boxCollider) {
    ...
    Vector3.transformMat4x4(boxCollider.boxMax, mat, max);
    Vector3.transformMat4x4(boxCollider.boxMin, mat, min);

    //--
    const temp: Vector3 = CollisionDetection._tempVec3;
    const corners = boxCollider.getCorners();
    for (let i = 0; i < 8; i++) {
      Vector3.transformMat4x4(corners[i], mat, temp);
  ...
  _getWorldSphere(sphereCollider) {
    ...
    Vector3.transformMat4x4(sphereCollider.center, sphereCollider.entity.transform.worldMatrix, center);
    return {
