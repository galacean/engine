...
export class SlicedSpriteAssembler {
  ...
  static updatePositions(renderer: SpriteRenderer): void {
    ...
    for (let i = 0, o = chunk.primitive.vertexBufferBindings[0].offset / 4; i < 4; i++) {
  ...
  static updateUVs(renderer: SpriteRenderer): void {
    ...
    for (let i = 0, o = chunk.primitive.vertexBufferBindings[0].offset / 4 + 3; i < 4; i++) {
  ...
  static updateColor(renderer: SpriteRenderer): void {
    ...
    const vertices = chunk.data._vertices;
    for (let i = 0, o = chunk.primitive.vertexBufferBindings[0].offset / 4 + 5; i < 16; ++i, o += 9) {
      vertices[o] = r;
      vertices[o + 1] = g;
      vertices[o + 2] = b;
      vertices[o + 3] = a;
