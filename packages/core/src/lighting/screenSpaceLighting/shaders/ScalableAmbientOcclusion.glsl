...
uniform sampler2D camera_DepthTexture;
    ...
    void computeAmbientOcclusionSAO(inout float occlusion, float i, float ssDiskRadius, vec2 uv, vec3 originPosition, vec3 normal,
        ...
        float occlusionDepth = texture2D(camera_DepthTexture, uvSamplePos).r;
...
void main(){
    float aoVisibility = 0.0;
            ...
            // Compute normal
            vec3 normal = computeViewSpaceNormal(v_uv, camera_DepthTexture, depth, material_invProjScaleXY, viewPos, renderer_texelSize.xy);
