...
export class PrimitiveMesh {
  ...
  static createCapsule(
    ...
    for (let i = 0; i < capVertexCount; ++i) {
      ...
      let posX = -radius * Math.cos(alphaDelta) * sinTheta;
      let posY = radius * Math.cos(thetaDelta) + halfHeight;
      let posZ = radius * Math.sin(alphaDelta) * sinTheta;

      // Position
      positions[i + torsoVertexCount] = new Vector3(posX, posY, posZ);
      // Normal
      normals[i + torsoVertexCount] = new Vector3(posX, posY, posZ);
      // Texcoord
      uvs[i + torsoVertexCount] = new Vector2(u, v);
    }
    ...
    for (let i = 0; i < capRectangleCount; ++i) {
      ...
      const a = y * radialCount + x + torsoVertexCount;
      const b = a + 1;
      const c = a + radialCount;
      const d = c + 1;
    ...
    for (let i = 0; i < capVertexCount; ++i) {
      ...
      // Position
      positions[i + torsoVertexCount + capVertexCount] = new Vector3(posX, posY, posZ);
      // Normal
      normals[i + torsoVertexCount + capVertexCount] = new Vector3(posX, posY, posZ);
      // Texcoord
      uvs[i + torsoVertexCount + capVertexCount] = new Vector2(u, v);
    }
    ...
    for (let i = 0; i < capRectangleCount; ++i) {
      ...
      const a = y * radialCount + x + torsoVertexCount + capVertexCount;
      const b = a + 1;
      const c = a + radialCount;
      const d = c + 1;
