...
vec4 texture2D2DBicubic(sampler2D tex, vec2 coord, vec4 texSize){
    ...
    return weights[0].y * (weights[0].x * texture2D(tex, (ic + vec2(offsets[0].x, offsets[0].y) - 0.5) * texSize.xy)  +
                        	weights[1].x * texture2D(tex, (ic + vec2(offsets[1].x, offsets[0].y) - 0.5) * texSize.xy)) +
            weights[1].y * (weights[0].x * texture2D(tex, (ic + vec2(offsets[0].x, offsets[1].y) - 0.5) * texSize.xy)  +
                            weights[1].x * texture2D(tex, (ic + vec2(offsets[1].x, offsets[1].y) - 0.5) * texSize.xy));
}
