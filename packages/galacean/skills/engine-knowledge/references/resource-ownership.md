...
- ResourceManager keys its cache and in-flight loads by that resolved URL, so a repeat load reuses the cached resource or the pending load instead of requesting again. Whether a completed load is cached depends on its loader; in-flight dedupe always applies, and only a load that misses both reaches the Loader.
