...
class KHR_materials_pbrSpecularGlossiness extends GLTFExtensionParser {
  override createAndParse(
    ...
    if (diffuseTexture) {
      context.get<Texture2D>(GLTFParserType.Texture, diffuseTexture.index, true).then((texture) => {
    ...
    if (specularGlossinessTexture) {
      ...
      context.get<Texture2D>(GLTFParserType.Texture, specularGlossinessTexture.index, true).then((texture) => {
