...
function _parseUniformParam(cfg: Prop): Prop {
  ...
  for (const name in cfg.uniforms) {
    ...
    if (UniformSemantic[semantic]) uniforms[name].semantic = UniformSemantic[semantic];
  }
...
export class TechniqueHandler {
  ...
  open(resource: Resource) {
    ...
    if (
      configJSON.hasOwnProperty("technique") &&
      ...
      configJSON.techniques &&
        configJSON.techniques.forEach(technique => {
