...
export function drawBezierLine( context, startPoint, controlPoint, config ) {
  ...
  context.beginPath();
  context.moveTo( startPoint.x, startPoint.y );
  context.bezierCurveTo( controlPoint.cp1x, controlPoint.cp1y, controlPoint.cp2x, controlPoint.cp2y, controlPoint.x, controlPoint.y );
  Object.assign( context, config );
  context.stroke();
  context.restore();
...
export function drawTextAutoLine( context, value, position, config ){
  ...
  for( let i = 0; i < value.length; i++ ){
    ...
    if( lineWidth > canvasWidth - context.measureText( value[i] ).width - initX ){//减去initX,防止边界出现的问题
