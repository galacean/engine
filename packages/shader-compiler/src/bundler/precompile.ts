...
function emitIndex(outputDir: string): void {
  ...
  // Two optional header lines:
  // 1. `/// <reference path="./global.d.ts" />` — only when `libs/global.d.ts`
  //    actually exists. The host package's tsconfig typically scopes
  //    `include` to `src/**/*`, so opening this auto-generated file directly
  //    in an IDE makes it a "loose file" with no ambient declarations
  //    visible — the `*.gsp` import then squiggles. The triple-slash
  //    directive forces TS Server to load the sibling `global.d.ts`
  //    regardless of project membership. Engine-style packages that
  //    declare `*.gsp` in `src/global.d.ts` (loaded via tsconfig) don't
  //    need a `libs/global.d.ts`, so we skip the reference there to avoid
  //    a "file not found" error from `tsc`.
  // 2. `// prettier-ignore` on the export block — the consumer's prettier
  //    config (printWidth, trailingComma, …) is unknown, so the multi-line
  //    block would otherwise drift on every precompile vs. format-on-save.
  //    Imports above are sorted deterministically and stay format-stable.
  const hasLibsGlobalDts = fs.existsSync(path.join(outputDir, "global.d.ts"));
  const refLine = hasLibsGlobalDts ? `/// <reference path="./global.d.ts" />\n` : "";
