...
function emitIndex(outputDir: string): void {
  ...
  // We don't know the consumer's prettier config (printWidth, trailingComma,
  // etc.), so the export block's format would otherwise drift on every
  // precompile vs. format-on-save. `// prettier-ignore` opts the next
  // statement out of formatting; imports are already sorted by varName so
