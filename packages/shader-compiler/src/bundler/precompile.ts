...
function emitIndex(outputDir: string): void {
  ...
  // Prefer single-line `export { A, B };` when it fits prettier's typical
  // 120-column wrap, otherwise emit a multi-line block. Matches what prettier
  // would do on the file, so format-on-save / lint-staged stays a no-op.
  const singleLine = `export { ${entries.map((e) => e.varName).join(", ")} };`;
  const exportBlock =
