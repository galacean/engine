...
export class ShaderLab implements IShaderLab {
  ...
  parseShader(shaderSource: string) {
    ...
    parsingContext.filterString(ParsingContext.editorPropertiesRegex);
    parsingContext.filterString(ParsingContext.editorMacrosRegex);
    ...
    if (this._parser.errors.length > 0) {
      for (const err of this._parser.errors) {
        const offset = parsingContext.getTextLinenOffsetAt(err.token.startOffset);
        if (offset) {
