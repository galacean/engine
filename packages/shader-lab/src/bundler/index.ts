...
export interface ShaderLabPrecompileOptions {
  ...
  emitIndex?: boolean;
  /** ShaderLab platform target. Default `0`. */
...
export interface ShaderLabPluginOptions {
   ...
   * must be triggered separately via the `shaderlab-precompile` CLI.
   */
  precompile?: ShaderLabPrecompileOptions;
}

/**
 * Rollup plugin that transforms ShaderLab assets into JS modules.
 *
 ...
 */
export function shaderLab(options: ShaderLabPluginOptions = {}): Plugin {
  const { filter, precompile } = options;
  const precompileOptions: ShaderLabPrecompileOptions | undefined = precompile && {
    clean: true,
    emitIndex: true,
    ...precompile
  };
  ...
  return {
    ...
    async buildStart() {
      ...
      if (this.meta.watchMode && !watcherStarted) {
        watcherStarted = true;
        startWatcher(precompileOptions).catch((e) => this.warn(`[shaderlab] watcher failed: ${(e as Error).message}`));
      }
    },
