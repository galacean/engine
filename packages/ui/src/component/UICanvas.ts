...
import { Mask } from "./advanced/Mask";
...
export class UICanvas extends Component implements IElement {
  ...
  private _adapterPoseInScreenSpace(): void {
    ...
    if (realRenderMode === CanvasRenderMode.ScreenSpaceCamera) {
      const cameraEntity = this._camera.entity;
  ...
  private _adapterSizeInScreenSpace(): void {
    ...
    if (realRenderMode === CanvasRenderMode.ScreenSpaceCamera) {
      ...
      curHeight = camera.isOrthographic
        ? camera.orthographicSize * 2
        : 2 * (Math.tan(MathUtil.degreeToRadian(camera.fieldOfView * 0.5)) * this._distance);
  ...
  private _walk(
    ...
    for (let i = 0, n = components.length; i < n; i++) {
      ...
      if (component instanceof Mask) {
        // Mask is a UIRenderer — process it as such, but also flag the mask
        hasMask = true;
        ...
        component._setRectMasks(tempRectMaskList, rectMaskCount);
        component._uiStencilDepth = stencilDepth;
    ...
    // If this entity has a Mask, increment stencil depth for children
    if (hasMask) {
      stencilDepth++;
    }
    ...
    for (let i = 0, n = children.length; i < n; i++) {
      ...
      child.isActive && (depth = this._walk(child, renderers, depth, group, rectMaskCount, stencilDepth));
    }
    return depth;
  }
