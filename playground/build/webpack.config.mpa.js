...
const srcPath = "../src";
const pages = fs.readdirSync(path.resolve(__dirname, srcPath)).filter(pageName => {
  let pagePath = path.resolve(__dirname, srcPath, pageName);
  let stats = fs.statSync(pagePath);
  if (stats.isDirectory()) {
    let files = fs.readdirSync(pagePath);
    return files.some(fileName => fileName === "index.ts");
  }
});

let entry = {};
pages.forEach(pageName => {
  entry[pageName] = [`./${pageName}/index.ts`];
});
let itemList = pages
...
let config = {
  ...
  module: {
    rules: [
      ...
      {
        test: /\.(png|jpe?g|gif|svg|bmp|tga)(\?.*)?$/i,
        use: [
          {
            loader: "url-loader",
            options: {
