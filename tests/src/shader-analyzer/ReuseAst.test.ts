...
import { GLES300Visitor, ShaderCompiler } from "@galacean/engine-shader-compiler";
...
describe("analyze exposes reusable AST (editor parses once)", () => {
  it("returns the parsed program(s) with no error diagnostics", () => {
    ...
    expect(diagnostics.filter((d) => d.severity === "error").map((d) => d.message)).to.deep.equal([]);
  ...
  it("codegen on the reused AST is identical to a fresh parse + codegen", () => {
    // Editor path: one parse via analyze(), then codegen the returned program — no re-parse.
    const { passes } = new ShaderAnalyzer().analyze(source);
    const reused = GLES300Visitor.getVisitor().visitShaderProgram(
      passes[0].program,
    ...
    // Reference path: structure-parse, then a fresh per-pass parse + codegen of the same source.
    ShaderCompilerUtils.clearAllShaderCompilerObjectPool();
    const p = ShaderSourceParser.parse(source).subShaders[0].passes[0];
    const fresh = new ShaderCompiler()._parseShaderPass(
      p.contents,
      p.vertexEntry,
      p.fragmentEntry,
