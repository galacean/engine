...
P1 已把动态局部效果拆成 `WaterLocalModifier`、有界 Interaction Event Queue 和 `TemporalFoamField`。静态 River Atlas 与动态 Pool deformation 可以通过同一 Composer 采样，但有时间历史的泡沫仍由独立 field 拥有，不能塞回不可变 `WaterWaveAsset`。
