...
River and Pool are functional-only regression cases during rapid water-system iteration. Ocean and Grasslands have separately reviewed, tracked Showcase Goldens; their pixel comparisons supplement the Showcase Smoke, Camera, Feature A/B, Performance, and Lifecycle gates instead of replacing them. The optional tracked Golden lane is not part of routine functional regression.

Visual `capture` is the non-mutating candidate path. The default `compare` checks every approved Golden; `WATER_PCG_VISUAL_CASE` can select one approved case. `update` is limited to exactly one explicit approved case and requires a review reason of at least twelve characters plus `WATER_PCG_VISUAL_UPDATE_APPROVAL=approved:<case-id>` after human approval. The update preflights every tracked PNG, stages the complete baseline in a sibling transaction directory, and only then swaps the baseline root with rollback on failure. Candidate captures use:
...
Ocean and Grasslands `compare` use their reviewed tracked baselines. Any future update to either case still requires a new case-specific human approval; one case's approval never authorizes another case or a multi-case update. River and Pool remain explicitly retired from screenshot equality. The optional Grasslands shared lane intentionally uses seed `20260723`; the dedicated M3 Reference Parity/8-state Regression lane uses seed `20260724`, so their screenshots are independent evidence and must not be substituted for one another.
