...
Showcase visual `capture` is the non-mutating candidate path and may target a newly promoted case that has no reviewed baseline. `compare` remains fail-closed until all selected cases exist in the tracked manifest. `update` requires exactly one explicit `WATER_PCG_VISUAL_CASE`, a review reason of at least twelve characters, and `WATER_PCG_VISUAL_UPDATE_APPROVAL=approved:<case-id>` after human approval. The update preflights every tracked PNG, stages the complete baseline in a sibling transaction directory, verifies that unselected case entries and hashes are unchanged, and only then swaps the baseline root with rollback on failure. An initial Grasslands candidate therefore uses:
